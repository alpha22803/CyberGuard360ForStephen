saqa = [
    ...
    "How do you manage user identifications and passwords?",
    ...
    "How do you protect your systems against malware?",
    ...
    "How often is the risk assessment process conducted?",
    ...
    "How do you protect cardholder data on public networks?",
...
saqb = [
    ...
    "How are new security vulnerabilities identified?",
    ...
    "How are group accounts managed for exceptional circumstances?",
    "How are shared accounts managed for exceptional circumstances?",
    ...
    "How are hard-copy materials with cardholder data destroyed?",
...
saqc = [
    ...
    "How are vendor default accounts managed if they will be used?",
    "How are vendor default accounts managed if they will not be used?",
    "How are primary functions requiring different security levels managed on a system component?",
    ...
    "How are system security parameters configured to prevent misuse?",
    ...
    "How are wireless encryption keys changed for these environments?",
    ...
    "How is SAD rendered unrecoverable upon completion of the authorization process?",
    ...
    "How is PAN masked when displayed?",
    "How is strong cryptography and security protocols implemented to safeguard PAN during transmission over open, public networks?",
    ...
    "How is the encryption strength ensured to be appropriate for the encryption methodology in use?",
    ...
    "For system components not at risk for malware, how are they evaluated periodically?",
    "What is the frequency of periodic evaluations for these system components?",
    "Is the anti-malware solution kept current via automatic updates?",
    "How does the anti-malware solution perform periodic and active or real-time scans?",
    "If periodic malware scans are performed, what is their frequency?",
    "For removable electronic media, how does the anti-malware solution perform scans?",
    ...
    "If manual code reviews are performed for bespoke and custom software, how are they managed?",
    "Are software engineering techniques or other methods used to prevent or mitigate common software attacks?",
    "How are security vulnerabilities identified and managed?",
    "Are new security vulnerabilities identified using industry-recognized sources?",
    "How are vulnerabilities assigned a risk ranking?",
    "Are vulnerabilities for bespoke and custom, and third-party software covered?",
    "How are system components protected from known vulnerabilities?",
    "How are critical or high-security patches/updates installed?",
    "Are changes to system components in the production environment made according to established procedures?",
    "How is the security impact of changes documented?",
    "Are changes tested to verify that they do not adversely impact system security?",
    "How are failures addressed to return to a secure state?",
    "Upon completion of a significant change, are all applicable PCI DSS requirements confirmed to be in place?",
    "How is access assigned to users based on job classification and function?",
    "Are required privileges approved by authorized personnel?",
    "How are user accounts and related access privileges reviewed?",
    "How are application and system accounts and related access privileges assigned and managed?",
    ...
    "How are group, shared, or generic accounts, or other shared authentication credentials managed?",
    "How is the addition, deletion, and modification of user IDs, authentication factors, and other identifier objects managed?",
    ...
    "How are accounts used by third parties for remote access managed?",
    "Is re-authentication required if a user session has been idle for more than 15 minutes?",
    "How is all user access to system components authenticated?",
    "Is strong cryptography used to render all authentication factors unreadable during transmission and storage?",
    "How is user identity verified before modifying any authentication factor?",
    "How are invalid authentication attempts limited?",
    "How are passwords/passphrases set and reset for each user?",
    ...
    "How are passwords/passphrases managed for single-factor authentication?",
    ...
    "How are MFA systems implemented to ensure security?",
    "How are accounts used by systems or applications for interactive login managed?",
    "Are passwords/passphrases for application and system accounts hard coded in scripts or source code?",
    "How are passwords/passphrases for application and system accounts protected against misuse?",
    ...
    "How is all media with cardholder data physically secured?",
    "How are offline media backups with cardholder data stored?",
    "How is media with cardholder data classified?",
    "How is media with cardholder data secured when sent outside the facility?",
    "How does management approve media with cardholder data moved outside the facility?",
    "How are hard-copy materials with cardholder data destroyed when no longer needed?",
    "How are POI devices protected from tampering and unauthorized substitution?",
    "How is an up-to-date list of POI devices maintained?",
    "How are POI device surfaces periodically inspected for tampering?",
    "What training is provided to personnel in POI environments?",
    "Are all security policies and operational procedures identified in Requirement 10 documented?",
    "Are these documents kept up to date?",
    "Are these policies and procedures in use and known to all affected parties?",
    "How are audit logs captured for actions taken by individuals with administrative access?",
    ...
    "What details are recorded in audit logs for each auditable event?",
    ...
    "What audit logs are reviewed at least once daily?",
    "Are automated mechanisms used to perform audit log reviews?",
    "How often are logs of other system components reviewed?"
]
